...
func newLifecycleTestServer(t *testing.T, busID uint32) (*usbServerHandleWrapper, *virtualbus.VirtualBus) {
	t.Helper()
	s := usb.New(usb.ServerConfig{Addr: "127.0.0.1:0", ConnectionTimeout: time.Hour, BusCleanupTimeout: time.Hour, DisableAutoBusCleanup: true, ManagedTransportLifecycle: true}, slog.Default(), nil)
