...
type steamControllerState struct {
	A, X, B, Y, L1, R1, Menu, Steam, Options, DPadDown, DPadLeft, DPadRight, DPadUp, L3, LGrip, RGrip, LPadTouch, RPadTouch, LPadPress, RPadPress, LPadAndStick bool
...
func steamControllerInputState(state steamControllerState) *steamcontroller.InputState {
	return &steamcontroller.InputState{
		A: state.A, X: state.X, B: state.B, Y: state.Y, L1: state.L1, R1: state.R1, Menu: state.Menu, Steam: state.Steam, Options: state.Options,
