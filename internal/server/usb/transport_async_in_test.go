...
func TestManagedDeviceDrainWaitsForINTransfer(t *testing.T) {
	for _, async := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			case <-time.After(time.Second):
				t.Fatal("non-EP0 IN transfer did not start")
			}
			...
			case <-dev.cancelled:
			case <-time.After(time.Second):
				t.Fatal("non-EP0 IN transfer did not observe cancellation")
			}
			select {
			case <-drainDone:
				t.Fatal("drain completed before the blocked IN transfer exited")
			default:
			}
			close(dev.release)
			select {
			case <-dev.exited:
			case <-time.After(time.Second):
				t.Fatal("non-EP0 IN transfer did not exit")
			}
			select {
			case <-drainDone:
			case <-time.After(time.Second):
				t.Fatal("drain did not complete after the IN transfer exited")
			}
